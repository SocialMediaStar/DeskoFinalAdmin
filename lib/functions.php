...
function decodePersonalCode($personalCode) {
    $gender = intval($personalCode[0]) % 2 === 0 ? 'Naine' : 'Mees';
    $birthYear = intval(substr($personalCode, 1, 2));
    $birthMonth = intval(substr($personalCode, 3, 2));
    $birthDay = intval(substr($personalCode, 5, 2));
    $birthDate = DateTime::createFromFormat('Y-m-d', "19{$birthYear}-{$birthMonth}-{$birthDay}");
    $now = new DateTime();
